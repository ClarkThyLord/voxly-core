## Example scene setup camera controller.
##
## Provides an orbit camera around a pivot, auto-rotation when idle, and
## mouse drag / scroll-wheel zoom. Double-clicking resets the view.
@tool
extends Node3D
...
@export var respect_scene_camera := true
...
## Runtime copy of the auto_rotate export.
var _auto_rotate := true
## True while the user is dragging the camera.
var _is_dragging := false
## The camera found under the pivot.
var _camera: Camera3D = null
## Current distance from the camera to the pivot.
...
# the scene camera.
## Scene-authored pivot transform restored by double-click reset.
var _initial_pivot_transform: Transform3D
## Scene-authored camera transform restored by double-click reset.
var _initial_camera_transform: Transform3D

# Snapshot of initial state for double-click reset.
## Initial pivot rotation restored by double-click reset.
var _initial_pivot_rotation: float = 0.0
## Initial camera distance restored by double-click reset.
var _initial_camera_distance: float = 5.0
## Initial vertical orbit angle restored by double-click reset.
var _initial_orbit_vertical: float = -0.35

## The pivot node the orbit camera rotates around.
@onready var camera_pivot: Node3D = %CameraPivot

## Initializes orbit tracking and camera state on play.
func _ready() -> void:
...
## Applies idle auto-rotation when not dragging.
...
## Routes mouse input to the orbit and zoom handlers.
...
## Handles mouse button input for orbit drag, zoom, and double-click reset.
func _handle_mouse_button(event: InputEventMouseButton) -> void:
	match event.button_index:
		...
		MOUSE_BUTTON_WHEEL_UP:
			_camera_distance = clampf(_camera_distance - zoom_step, zoom_min, zoom_max)
			_update_camera_position()
			get_viewport().set_input_as_handled()
...
## Handles mouse motion for orbit dragging around the pivot.
func _handle_mouse_motion(event: InputEventMouseMotion) -> void:
	...
	_orbit_vertical = clampf(_orbit_vertical - delta.y, -1.55, 1.55)
...
func _update_camera_position() -> void:
	...
	_camera.position = orbit_position
...
func _sync_orbit_from_camera() -> void:
	...
	var orbit_position := _camera.position
	_camera_distance = clampf(orbit_position.length(), zoom_min, zoom_max)
	if _camera_distance > 0.001:
		_orbit_vertical = asin(clampf(orbit_position.y / _camera_distance, -1.0, 1.0))
