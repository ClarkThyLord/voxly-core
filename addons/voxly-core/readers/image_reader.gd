...
##
## Each non-transparent pixel becomes a voxel. The image is placed on the Z=0
## plane by default, with optional depth extrusion to create thicker voxel
## models.
@tool
class_name ImageReader
extends RefCounted

const _debug_context := "ImageReader"

## Maximum number of colors allowed in the auto-generated palette.
...
static func read(image: Image, options: Dictionary = {}) -> Dictionary:
	...
	depth = maxi(depth, 1)
	max_colors = clampi(max_colors, 1, MAX_PALETTE_COLORS)
	
	VoxlyDebug.log_category(VoxlyDebug.CATEGORY_READERS, _debug_context, "Reading image: %dx%d, depth=%d, max_colors=%d" % [image.get_width(), image.get_height(), depth, max_colors])
	...
	VoxlyDebug.log_category(VoxlyDebug.CATEGORY_READERS, _debug_context, "Image palette: %d colors from image" % result["palette"].size())
	
	# Quantize palette if needed.
	if result["palette"].size() > max_colors:
		_quantize_palette(result, color_to_id, max_colors)
		VoxlyDebug.log_category(VoxlyDebug.CATEGORY_READERS, _debug_context, "Quantized palette to %d colors" % result["palette"].size())
	...
	VoxlyDebug.log_category(VoxlyDebug.CATEGORY_READERS, _debug_context, "Generated %d voxels from image" % result["voxels"].size())
...
static func read_file(file_path: String, options: Dictionary = {}) -> Dictionary:
	VoxlyDebug.log_category(VoxlyDebug.CATEGORY_READERS, _debug_context, "Reading image file: '%s'" % file_path)
	
	# Read file bytes directly and load via the format-specific loader.
	var file := FileAccess.open(file_path, FileAccess.READ)
	if file == null:
		VoxlyDebug.log_category(VoxlyDebug.CATEGORY_READERS, _debug_context, "Failed to open image file: '%s'" % file_path)
	...
	match extension:
		...
		_:
			# Fallback: try a direct file load.
	...
	if load_error != OK or image == null or image.is_empty():
		VoxlyDebug.log_category(VoxlyDebug.CATEGORY_READERS, _debug_context, "Failed to load image file: '%s'" % file_path)
		return { "error": ERR_FILE_CANT_OPEN, "voxels": {}, "palette": {} }
	...
	if result.get("error") == OK:
		VoxlyDebug.log_category(VoxlyDebug.CATEGORY_READERS, _debug_context, "Image read OK: %d voxels, %d palette colors" % [result.get("voxels", {}).size(), result.get("palette", {}).size()])
...
static func _find_nearest_color(color_to_id: Dictionary, target: Color) -> int:
	...
	var target_r := target.r8
	var target_g := target.g8
	var target_b := target.b8
	for key in color_to_id:
		var parts: PackedStringArray = key.split("_")
		if parts.size() >= 3:
			var key_r := int(parts[0])
			var key_g := int(parts[1])
			var key_b := int(parts[2])
			var delta_r := target_r - key_r
			var delta_g := target_g - key_g
			var delta_b := target_b - key_b
			var dist := delta_r * delta_r + delta_g * delta_g + delta_b * delta_b
			if dist < best_dist:
				best_dist = dist
...
static func _quantize_palette(result: Dictionary, color_to_id: Dictionary, max_colors: int) -> void:
	...
	while palette.size() > max_colors and step < 256:
		...
		for voxel_id in palette:
			...
			var color := voxel.base_color
			var r := roundi(color.r8 / step) * step
			var g := roundi(color.g8 / step) * step
			var b := roundi(color.b8 / step) * step
			var quantized_key := "%d_%d_%d_255" % [r, g, b]
			
			if not quantized_map.has(quantized_key):
				var quantized_voxel := Voxel.new()
				quantized_voxel.base_color = Color8(r, g, b, 255)
				quantized_voxel.name = "quantized_%d" % next_id
				new_palette[next_id] = quantized_voxel
				new_color_to_id[quantized_key] = next_id
				quantized_map[quantized_key] = next_id
				next_id += 1
		
		# Update the palette.
		palette.clear()
		for voxel_id in new_palette:
			palette[voxel_id] = new_palette[voxel_id]
		
		# Update the color_to_id mapping.
		color_to_id.clear()
		for key in new_color_to_id:
			color_to_id[key] = new_color_to_id[key]
		
		step *= 2  # Increase the step for the next iteration if still too many colors.
