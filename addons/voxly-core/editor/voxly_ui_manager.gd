## Manages the lifecycle of editor docks for the Voxly editor.
##
## Supports simultaneous display: the VoxelSet editor lives in a side editor dock,
## while the VoxelNode3D editor lives in a bottom editor dock.
##
...
## it was dismissed (switched to the Inspector, another dock/tab in its slot, or
## closed via the context popup). While a dock is dismissed, node selection
## syncs its content without re-opening it; however, actions (clicking its
## tab/button, Editor > Editor Docks) or a context change (deselect, non-voxel
## selection, leaving 3D) clear the dismissal. Ending the editing context fully
## closes the docks (tabs removed); the cached instances are still reused and
## reopen in their remembered slots on the next voxel selection.
##
...
const _debug_context := "VoxlyUIManager"
...
## Emitted when a dock is shown.
signal dock_shown(dock_type: String)

## Emitted when a dock is hidden.
...
## The editor plugin owning this UI manager.
var _editor_plugin: EditorPlugin
## The state machine driving dock visibility.
var _state_machine: VoxlyStateMachine
## The editor input handler shared with the docks.
var _input_handler: VoxlyInputHandler

## Currently active bottom dock Control node.
var _bottom_dock: Control = null
## The dock type currently shown in the bottom panel.
var _bottom_dock_type: DockType = DockType.NONE

## VoxelSet editor dock, managed separately so it can coexist with the bottom dock.
var _voxel_set_dock: EditorDock = null

## Spawned docks, keyed by scene path. Docks are created once and reused.
var _spawned_docks: Dictionary[String, Control] = {}

## Docks already added to the editor, keyed by scene path (true when registered).
var _added_docks: Dictionary[String, bool] = {}
...
var _dock_dismissed: Dictionary[String, bool] = {}
...
## before the dock has ever been shown; only a visible-then-hidden dock counts
## as user-dismissed.
var _dock_has_been_shown: Dictionary[String, bool] = {}

## Guards signal handlers from treating our own programmatic show/hide calls as
## user actions.
var _setting_docks_up: bool = false

## Guards signal handlers from treating our own close()/make_visible() calls as
## user dismissals. Dismissal is only recorded when the user moves focus away
## (tab switch, another dock in the slot, context-popup close), never our own
## programmatic sync.
var _syncing_docks: bool = false

## The active VoxelModel3DController.
...
## Stores the plugin and state machine, then builds the dock lifecycle.
...
## Returns the currently active bottom dock control.
func get_current_dock() -> Control:
	return _bottom_dock

## Returns the type of the currently active bottom dock.
...
## Shows a dock for the given dock type and optional node.
...
## Should only be called on plugin exit: disposes all docks and resets the
## spawn cache.
...
## opened via a node shares that context and is hidden too, while a standalone
## VoxelSet dock remains open across all views.
...
## Reacts to state transitions by showing, hiding, and syncing docks.
func _on_state_changed(transition: VoxlyState.Transition) -> void:
	# Leaving editing mode also restores the user's previous transform tool mode.
	...
	match transition.to_state:
		...
		VoxlyState.State.EDITING_VOXEL_MODEL:
			# Entering editing mode also remembers the user's transform tool and
			# forces Select mode so the transform gizmo doesn't fight painting.
			VoxlyDebug.log_category(VoxlyDebug.CATEGORY_EDITOR_UI, _debug_context,
...
## Syncs the voxel set dock content with the newly selected set.
...
## Hides and dismisses the voxel node dock.
func _hide_voxel_node_dock() -> void:
	...
	VoxlyDebug.log_category(VoxlyDebug.CATEGORY_EDITOR_UI, _debug_context, "Hidden %s dock" % DockType.keys()[old_type])

## Hides and dismisses the voxel set dock.
func _hide_voxel_set_dock() -> void:
	...
	VoxlyDebug.log_category(VoxlyDebug.CATEGORY_EDITOR_UI, _debug_context, "Hidden VoxelSet right-side dock")
...
##
## Programmatic hide always means the editing context ended (node deselected,
...
func _set_dock_visible(dock: Control, visible: bool, path: String) -> void:
	...
	if visible:
		...
		# which reopens the dock in its remembered slot. Clear dismissal and the
		# shown flag so a fresh context always starts with the docks available
		# again, and so any deferred visibility_changed(false) from close() is
		# ignored (it can't re-poison dismissal state).
...
func _connect_dock_signals(dock, path: String) -> void:
	...
	# Wire the VoxelSet dock's palette selection to the active editor palette.
	# This connection was lost in the dock rework: docks are spawned once and reused.
...
## Fires when a dock's visibility changes (tab switch, menu open/close; our own
## adds/removes/close()/make_visible() calls are guarded by _setting_docks_up
## and _syncing_docks). A dock only becomes "dismissed" when the user moves
## focus away from it; creation-time hidden states (before the dock was ever
## shown) never count.
...
## explicit user action, so the dock's content is always synced. It only opens
## automatically when the dock hasn't been dismissed (first use or explicit
## intent); a dismissed dock refreshes in the background while the user stays
## focused on whatever they were doing (e.g. the Inspector).
func _show_voxel_set_dock(node: Node3D) -> void:
	var voxel_set: VoxelSet = _state_machine.current_voxel_set
	if not voxel_set:
		return
	...
	VoxlyDebug.log_category(VoxlyDebug.CATEGORY_EDITOR_UI, _debug_context, "Synced VoxelSet editor right-side dock")
...
func _show_voxel_set_dock_for_node(node: VoxelNode3D) -> void:
	...
	var voxel_set: VoxelSet = node.voxel_set
	_state_machine.set_current_voxel_set(voxel_set)
	...
	if not _is_dock_dismissed(VOXEL_SET_DOCK_PATH):
		_set_dock_visible(dock, true, VOXEL_SET_DOCK_PATH)
	dock_shown.emit("VOXEL_SET_EDITOR")
	VoxlyDebug.log_category(VoxlyDebug.CATEGORY_EDITOR_UI, _debug_context, "Synced VoxelSet editor for node: %s" % node.name)
...
## Sets the palette voxel ID for the editor. Does not automatically enable
## editing; the user must toggle editing via the VoxelNode3DEditorDock checkbox.
...
func _on_voxel_set_dock_palette_changed(voxel_id: int) -> void:
	...
	if _is_voxel_set_dock_mismatched():
		# Dock shows a different VoxelSet than the active node, so don't apply
	...
	# Editing mode is intentionally not disabled when the palette is cleared;
	# the user can continue editing, and the editor dock's notice reminds them
...
func _sync_voxel_set_for_node(node: VoxelNode3D) -> void:
	...
	var voxel_set := node.voxel_set
	if _bottom_dock.has_method("set_voxel_set"):
		_bottom_dock.set_voxel_set(voxel_set)
	
	if _active_controller:
		_active_controller.editor.voxel_set = voxel_set
	
	# Show or hide VoxelSetEditorDock based on whether the node has a VoxelSet.
	if voxel_set:
		_show_voxel_set_dock_for_node(node)
	else:
		_hide_voxel_set_dock()

## Spawns and wires the VoxelNode3D editor dock for the given node, creating a
## controller for it and connecting all signals.
func _show_voxel_node_editor_dock(node: Node3D) -> void:
	...
	if node is VoxelModel3D:
		...
		VoxlyDebug.log_category(VoxlyDebug.CATEGORY_EDITOR_UI, _debug_context, "Unsupported node type: %s" % node.get_class())
		return
	...
	if not dock:
		VoxlyDebug.log_category(VoxlyDebug.CATEGORY_EDITOR_UI, _debug_context, "Failed to create VoxelNode3D editor dock")
		_cleanup_controller()
		return
	...
	var voxel_set = _active_controller.editor.voxel_set
	if dock.has_method("set_voxel_set"):
		dock.set_voxel_set(voxel_set)
	...
	# Connect the node signal for VoxelSet changes.
	...
	VoxlyDebug.log_category(VoxlyDebug.CATEGORY_EDITOR_UI, _debug_context, "Shown VoxelNode3D editor dock for: %s" % node.name)
	dock_shown.emit("VOXEL_OBJECT_EDITOR")

## Handles the "VoxelSet Editor" button request from the VoxelNode3DEditorDock.
...
## Handles the "Add VoxelSet" request from the VoxelNode3DEditorDock.
...
func _on_dock_add_voxel_set_requested(node: VoxelNode3D, dock) -> void:
	...
	if undo_redo:
		undo_redo.create_action("Add VoxelSet to %s" % node.name, UndoRedo.MERGE_DISABLE, node)
		undo_redo.add_do_property(node, "voxel_set", new_set)
		undo_redo.add_do_method(new_set, "set_voxel", first_id, new_voxel)
		undo_redo.add_undo_method(new_set, "remove_voxel", first_id)
		undo_redo.add_undo_property(node, "voxel_set", null)
...
## Syncs the dock editing toggle when editing state changes on the controller.
...
## Forwards editing toggles from the docks to the active controller.
...
## Syncs the voxel set dock when the edited node voxel set changes.
func _on_node_voxel_set_changed(node: VoxelNode3D, dock) -> void:
	...
	if voxel_set:
		...
		# Clear palette when the voxel set is removed.
		if _active_controller:
			_active_controller.editor.palette_id = -1
...
## Wires or unwires the active controller as the editing input handler.
func _wire_input_handler(enabled: bool) -> void:
	if not _input_handler:
		return
	
	if enabled and _active_controller:
		_input_handler.editing_input_handler = Callable(_active_controller, "handle_input")
		VoxlyDebug.log_category(VoxlyDebug.CATEGORY_EDITOR_UI, _debug_context, "Input handler wired to controller")
	else:
		_input_handler.editing_input_handler = Callable()
		VoxlyDebug.log_category(VoxlyDebug.CATEGORY_EDITOR_UI, _debug_context, "Input handler unwired")

## Stops the active controller and releases it.
