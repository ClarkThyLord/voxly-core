...
var _pending_undo_redo_manager: EditorUndoRedoManager = null
...
func set_voxel_set(voxel_set: VoxelSet) -> void:
	...
	# Disconnect from previous voxel set's changed signal
...
## Propagates the set to the editor, wires the changed signal, and
## auto-selects the first voxel so the dock opens with a valid selection.
