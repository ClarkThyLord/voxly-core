...
##
## Brushes, tools, the controller, and operation helpers all reach voxel
## data through this adapter instead of holding the raw node, such that:
## Gives UndoRedo a stable RefCounted to register methods on (RefCounted
## stays alive as long as the undo stack references it).
## Provides a single `is_valid()` guard, so a freed node can never crash
## a pending undo/redo operation.

var node = null

func _init(p_node = null) -> void:
	node = p_node
...
func voxel_at(pos: Vector3i):
	if not is_valid() or not node.has_method("get_voxel"):
		return null
	return node.get_voxel(pos)


## Writes (or removes, with null) a palette ID at the position.
func set_voxel(pos: Vector3i, voxel_id) -> void:
	if not is_valid():
		return
	if voxel_id == null:
		if node.has_method("remove_voxel"):
			node.remove_voxel(pos)
	else:
		node.set_voxel(pos, voxel_id)
...
## Bulk-applies a Dictionary[Vector3i, id] voxel map. Idempotent, so both
## do and undo can re-run the same call safely.
func apply_voxel_map(voxels: Dictionary) -> void:
	for pos in voxels:
		set_voxel(pos, voxels[pos])
...
func get_exposed_face_voxels(pos: Vector3i, normal: Vector3i, match_id: bool) -> Array[Vector3i]:
	if not is_valid() or not node.has_method("get_exposed_face_voxels"):
		return []
	return node.get_exposed_face_voxels(pos, normal, match_id)


## True when the position is inside the model's grid bounds.
func is_voxel_position_valid(pos: Vector3i) -> bool:
	if not is_valid() or not node.has_method("is_voxel_position_valid"):
		return true
	return node.is_voxel_position_valid(pos)


## DDA raycast against occupied voxels (for voxel-level hit resolution).
func voxel_raycast(from: Vector3, dir: Vector3, max_dist: float) -> Dictionary:
	if not is_valid() or not node.has_method("voxel_raycast"):
		return {}
	return node.voxel_raycast(from, dir, max_dist)
