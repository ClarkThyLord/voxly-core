...
func execute(editor, undo_redo: EditorUndoRedoManager) -> void:
	...
	for ax in axes:
		...
		match align_mode:
			0: shape_edge = 0
			1: shape_edge = (shape[ax] - 1) / 2
			_: shape_edge = shape[ax] - 1
		var current_edge: int
		match align_mode:
			0: current_edge = min_corner[ax]
			1: current_edge = (min_corner[ax] + max_corner[ax]) / 2
			_: current_edge = max_corner[ax]
		delta[ax] = shape_edge - current_edge
