...
## Flips the targeted voxels in place across the targeted region's
## center axis. Targets the selection when one exists, otherwise all filled voxels.
...
func execute(editor, undo_redo: EditorUndoRedoManager) -> void:
	...
	for pos in positions:
		var flipped := pos
		flipped[axis] = max_corner[axis] - (pos[axis] - min_corner[axis])
		if not seen.has(flipped) and flipped != pos:
			seen[flipped] = true
