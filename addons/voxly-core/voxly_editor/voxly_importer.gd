...
const DEBUG_CONTEXT := "VoxlyImporter"

## Reads a file for the in-editor import workflow.
## Wraps VoxlyReader.read_file, then normalizes the voxel positions so the
## minimum corner is at (0,0,0) and computes the content's bounding size.
## Returns:
##   {
##     error: int,
...
## Merges an imported palette + materials into an existing VoxelSet,
## remapping any conflicting IDs to fresh ones.
##   {
...
static func merge_into_voxel_set(existing: VoxelSet, palette: Dictionary, materials: Dictionary) -> Dictionary:
	...
	for source_mat_id in materials:
		var mat_data: Dictionary = materials[source_mat_id]
		var palette_color := Color.WHITE
		var pal_idx := int(source_mat_id) - 1 # MATL ids are 1-indexed; palette 0-indexed
		...
		var new_mat_id := str(source_mat_id)
		if new_materials.has(new_mat_id):
			var next := existing.next_material_id().to_int()
			while new_materials.has(str(next)):
				next += 1
			new_mat_id = str(next)
		material_remap[str(source_mat_id)] = new_mat_id
		new_materials[new_mat_id] = material
	...
	VoxlyDebug.log_category(VoxlyDebug.CATEGORY_EDITOR_LOGIC, DEBUG_CONTEXT,
...
## `merge_result` is what merge_into_voxel_set() returned.
##
...
static func register_voxel_set_merge(undo_redo: EditorUndoRedoManager, voxel_set: VoxelSet, merge_result: Dictionary) -> void:
	...
	# Materials
	for mat_id in new_materials:
		if old_materials.has(mat_id):
			undo_redo.add_do_method(voxel_set, "set_material", mat_id, new_materials[mat_id])
			undo_redo.add_undo_method(voxel_set, "set_material", mat_id, old_materials[mat_id])
		else:
			undo_redo.add_do_method(voxel_set, "set_material", mat_id, new_materials[mat_id])
...
static func apply_voxels_undoable(undo_redo: EditorUndoRedoManager, target, voxel_map: Dictionary) -> void:
	for pos in voxel_map:
		var old_id = target.get_voxel(pos)
		undo_redo.add_do_method(target, "set_voxel", pos, voxel_map[pos])
		if old_id != null:
