...
## hovered voxel. Acts as a region selector; the active tool applies its
...
## When true, only spread through voxels with the same ID as the clicked voxel
## (standard flood fill). When false (default), spread through the entire
## connected region regardless of voxel ID. Clicking empty space always floods
## the connected empty region.
var match_id: bool = true

## When true, the region is recomputed every frame and ghosted in the viewport
## preview. Flood fill can be expensive on large models, but the preview is
## capped by [member preview_voxel_limit] so it stays responsive.
var enable_preview: bool = true

## Maximum number of voxels to generate during hover preview so it stays
## responsive on large models. Only affects the preview, never the actual
## operation.
var preview_voxel_limit: int = 2048

## Registers the fill brush in the registry.
func _init() -> void:
	name = "fill"
...
## Returns all connected matching voxels (flood fill).
...
## Returns the color used for the fill preview.
...
## Returns whether the fill shows a preview.
func show_preview() -> bool:
	return enable_preview

## Resolves the position the fill starts from for the current hit.
## Returns [constant Vector3i.MAX] if there is no usable hit.
func _resolve_start_position(editor: VoxlyEditor, hit: Dictionary) -> Vector3i:
	if hit.is_empty():
		return Vector3i.MAX
	...
	if not adapter:
		return Vector3i.MAX
	if not adapter.is_voxel_position_valid(position):
		return Vector3i.MAX
	return position

## Performs a flood fill starting from the given position.
## When [param use_match_id] is true, only spreads through voxels whose ID
## equals [param target_id] (flooding through empty space when clicking air).
## When false, spreads through the entire connected region of the same
## occupancy (filled or empty) regardless of voxel ID.
## When [param max_voxels] > 0, stops collecting after that many positions.
