...
## relative to a hit position. The tool acts on those positions.
...
## Whether this brush supports the continuous painting mode.
...
func offset_for_tool(editor, pos: Vector3i, normal: Vector3i, is_dda_hit: bool) -> Vector3i:
	if editor and editor.active_tool and is_dda_hit:
		return editor.active_tool.offset_position(pos, normal)
	return pos
...
## Called during a drag to update positions from origin to current hit.
