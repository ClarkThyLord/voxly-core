## Base controller that bridges a [VoxlyEditor] instance with a target voxel
## node. Handles the lifecycle of attaching/detaching editor overlays,
## raycasting, input handling, and UI synchronization.
##
## Subclasses should override [method raycast], [method start_editing], and
## [method stop_editing].
@tool
@abstract
...
const _debug_context := "VoxelNode3DController"

## Config section name used to persist editor state through [VoxlyConfig].
const CONFIG_NAME := "voxel_node_3d_editor"

## Emitted when editing starts on a target.
signal editing_started(target)

## Emitted when editing stops.
signal editing_stopped

## Emitted when an input event was handled.
...
## The grid overlay node (attached to the target during editing).
var grid: VoxlyEditorGrid = null

## The preview overlay node.
var preview: VoxlyEditorPreview = null

## Cached reference to the VoxelSet editor UI.
var voxel_set_editor_ui = null

## Mouse-driven interaction models are [VoxlyInteraction]s, each owning one
## interaction lifecycle. The controller is a thin dispatcher: it resolves the
## interaction for the current brush/tool state on press and routes subsequent
## drag/up events to it.
var _interaction: VoxlyInteraction = null
## Stroke interaction for painting.
var _stroke_interaction: VoxlyStrokeInteraction = null
## Drag interaction for orbiting.
var _drag_interaction: VoxlyDragInteraction = null
## Click interaction for picking.
...
## Creates the interactions and connects editor signals.
func _init() -> void:
	editor = VoxlyEditor.new()

## Called when the controller should start editing a target node.
## Subclasses should call super(). Overlays and `_voxly_core_editing_` are not
## attached here; they are attached only when editing is toggled on via
## [method set_editing].
...
func set_editing(enabled: bool) -> void:
	...
	if not enabled:
		...
		if _interaction:
			...
			_interaction = null
		editor.cancel_stroke(editor.adapter)
		# Clear the selection when leaving editing mode so no stale highlighted
		# positions linger on the model.
		editor.selection.clear()
	...
	# While the node is merely selected (dock open, editing off), changes to
	# attach_static_body / create_static_body() take effect immediately.
...
## Attaches the grid and preview overlays to the target node as internal
## children so they do not appear in the scene tree dock.
func _attach_overlays() -> void:
	...
	if VoxlyDebug.is_category_enabled(VoxlyDebug.CATEGORY_EDITOR_LOGIC):
		VoxlyDebug.log_category(VoxlyDebug.CATEGORY_EDITOR_LOGIC, _debug_context, "Debug visuals enabled")
...
## is a USER preference (settings window), never clobbered here. We only
## clear any stale ghost when editing turns off.
...
##
## [param append] overlays the imported voxels at the origin (normalized
## positions, matching the user's chosen behavior). Otherwise the target's
## existing voxels are replaced. Palette entries and materials are merged into
## the target's VoxelSet (creating one if none exists) with conflicting IDs
...
##
## Runs as a chunked coroutine, so the editor stays responsive and the UI
## drives its progress window from the import_started / import_progress /
## import_finished signals.
func import_file(path: String, append: bool) -> void:
	if not is_instance_valid(target) or not undo_redo:
		VoxlyDebug.log_category(VoxlyDebug.CATEGORY_EDITOR_LOGIC, _debug_context, "import_file: no valid target/undo_redo")
	...
	if imported_voxels.is_empty():
		VoxlyDebug.log_category(VoxlyDebug.CATEGORY_EDITOR_LOGIC, _debug_context, "import_file: no voxels to import")
		import_finished.emit(false)
		return
	...
	for position in imported_voxels:
		var voxel_id: int = imported_voxels[position]
		final_voxels[position] = voxel_remap.get(voxel_id, voxel_id)
	...
	var written: int = 0
	for position in final_voxels:
		var old_id = target.get_voxel(position)
		undo_redo.add_do_method(target, "set_voxel", position, final_voxels[position])
		if old_id != null:
...
func _calc_import_shape(target, incoming_voxels: Dictionary, current_shape: Vector3i) -> Vector3i:
	...
	for position in incoming_voxels:
		var typed_position: Vector3i = position
		max_all.x = maxi(max_all.x, typed_position.x)
		max_all.y = maxi(max_all.y, typed_position.y)
		max_all.z = maxi(max_all.z, typed_position.z)
	# Include existing voxel extent (they may exceed current_shape).
	for position in target.get_voxels():
		var typed_position: Vector3i = position
		max_all.x = maxi(max_all.x, typed_position.x)
		max_all.y = maxi(max_all.y, typed_position.y)
		max_all.z = maxi(max_all.z, typed_position.z)
...
## Dual-approach raycasting:
## - Raycast against the cage collision (grid) for surface hits.
## - Or if the tool targets existing voxels, use the DDA voxel raycast.
##
...
func raycast(camera: Camera3D, screen_position: Vector2) -> Dictionary:
	...
	var voxel_position := Vector3i(
		floori(local_offset.x / voxel_size.x),
		floori(local_offset.y / voxel_size.y),
		floori(local_offset.z / voxel_size.z)
	)
	
	# Out-of-bounds correction for cage wall hits.
	var shape := grid.shape
	var out_of_bounds := (
		voxel_position.x < 0 or voxel_position.x >= shape.x or
		voxel_position.y < 0 or voxel_position.y >= shape.y or
		voxel_position.z < 0 or voxel_position.z >= shape.z
	)
	if out_of_bounds:
		voxel_position += Vector3i(hit_normal)
	...
	if _should_dda_raycast():
		...
		var dda_result: Dictionary = target.voxel_raycast(from, direction_global, 256.0)
...
## Routes 3D viewport input to the active interaction.
...
## Interaction instances are cached so hover previews can resolve fresh per
## event without losing per-interaction state.
...
## Picks the interaction for the current tool and state.
...
## Updates the debug ray overlay from the latest hit.
func _update_debug_ray(camera: Camera3D, screen_position: Vector2, hit: Dictionary) -> void:
	if not preview or not VoxlyDebug.is_category_enabled(VoxlyDebug.CATEGORY_RAYCAST):
		return
	
	preview.show_debug_ray(camera, screen_position)
	
	var from := camera.project_ray_origin(screen_position)
	var direction := camera.project_ray_normal(screen_position)
	var log_cat := VoxlyDebug.CATEGORY_RAYCAST
	
	VoxlyDebug.log_category(log_cat, _debug_context,
		"Camera world pos=(%.2f,%.2f,%.2f)" % [camera.global_position.x, camera.global_position.y, camera.global_position.z])
	VoxlyDebug.log_category(log_cat, _debug_context,
		"Ray origin=(%.2f,%.2f,%.2f) dir=(%.6f,%.6f,%.6f)" % [from.x, from.y, from.z, direction.x, direction.y, direction.z])
	
	var hit_pos_world := from + direction * 10000.0
	VoxlyDebug.log_category(log_cat, _debug_context,
		"Ray start=(%.2f,%.2f,%.2f) end=(%.2f,%.2f,%.2f)" % [from.x, from.y, from.z, hit_pos_world.x, hit_pos_world.y, hit_pos_world.z])
	
	var dda_positions := _compute_dda_positions(from, direction, hit)
...
## Standard DDA traversal. Returns an array of all cells the ray passes through.
func _compute_dda_positions(from: Vector3, direction: Vector3, hit: Dictionary) -> Array:
	if not grid:
		return []
	
	var voxel_size := grid.voxel_size
	var origin_offset: Vector3 = grid.origin_offset
	var local_from = target.to_local(from) - origin_offset
	var local_dir = target.to_local(from + direction) - target.to_local(from)
	var max_steps := 64
	
	var positions: Array = []
	var current := Vector3i(
		floori(local_from.x / voxel_size.x),
		floori(local_from.y / voxel_size.y),
		floori(local_from.z / voxel_size.z)
	)
	...
	var axes := ["x", "y", "z"]
	for i in 3:
		var direction_value = local_dir[axes[i]]
		if direction_value == 0:
			delta_dist[i] = INF
			side_dist[i] = INF
			continue
		var axis_voxel_size := voxel_size[axes[i]]
		delta_dist[i] = abs(axis_voxel_size / direction_value) # t to cross one voxel
		if direction_value < 0:
			step[i] = -1
			side_dist[i] = (local_from[axes[i]] - current[i] * axis_voxel_size) * abs(1.0 / direction_value)
		else:
			step[i] = 1
			side_dist[i] = ((current[i] + 1) * axis_voxel_size - local_from[axes[i]]) * abs(1.0 / direction_value)
...
## Updates the editor palette voxel ID.
func set_palette(voxel_id: int) -> void:
	editor.palette_id = voxel_id

## Returns the config path for this controller.
...
func load_config(config_path: String = "") -> void:
	...
	if state.has("mirrors") and typeof(state["mirrors"]) == TYPE_ARRAY:
		var mirrors: Array = state["mirrors"]
		if mirrors.size() >= 3:
			editor.set_mirrors(Vector3i(int(mirrors[0]), int(mirrors[1]), int(mirrors[2])))
...
## Stores a source's option values under `section` (i.e. brushes/tools) keyed
## by name.
...
func _collect_option_values(options: Array[Dictionary], source) -> Dictionary:
	...
	for option in options:
		var property: String = option.get("property", "")
