...
## Controller for editing VoxelModel3D nodes. Handles model-specific
## overlay configuration, raycasting, and UI population.
		...
		if undo_redo:
			...
			if editor and editor.selection and editor.selection.count() > 0:
				...
				for pos in old_selection:
					if pos.x >= 0 and pos.x < new_value.x \
							and pos.y >= 0 and pos.y < new_value.y \
							and pos.z >= 0 and pos.z < new_value.z:
						pruned.append(pos)
...
## Overrides the base raycast to model-specific adjustments.
func raycast(camera: Camera3D, screen_position: Vector2) -> Dictionary:
	var result := super.raycast(camera, screen_position)
	if result.is_empty():
		return result
	
	# Validate the position is within the model bounds
	if is_instance_valid(model):
		var pos: Vector3i = result.get("position", Vector3i.ZERO)
		if not model.is_voxel_position_valid(pos):
			# Clamp to nearest valid position on the cage face
			var normal: Vector3i = result.get("normal", Vector3i.ZERO)
			pos = _clamp_to_shape(pos, normal)
			result["position"] = pos
...
func _clamp_to_shape(pos: Vector3i, normal: Vector3i) -> Vector3i:
	if not is_instance_valid(model):
		return pos
	
	var shape := model.shape
	# Clamp each axis to valid range, accounting for face normal
	pos.x = clampi(pos.x, -1 if normal.x == -1 else 0, shape.x - (1 if normal.x == 1 else 0))
	pos.y = clampi(pos.y, -1 if normal.y == -1 else 0, shape.y - (1 if normal.y == 1 else 0))
	pos.z = clampi(pos.z, -1 if normal.z == -1 else 0, shape.z - (1 if normal.z == 1 else 0))
	return pos
...
func populate_brush_ui(brush_container) -> void:
	...
	for name in brush_names:
		...
		btn.button_pressed = (name == editor.brush_name)
		
		# Apply icon from the brush instance if available
		var brush := editor.registry.create_brush(name)
		if brush and brush.icon:
			btn.icon = brush.icon
		
		btn.pressed.connect(_on_brush_ui_selected.bind(name, brush_container))
		brush_container.add_child(btn)
	
	# Listen for brush changes to update UI
...
func populate_tool_ui(tool_container) -> void:
	...
	for name in tool_names:
		...
		btn.text = name.capitalize()
		btn.toggle_mode = true
		btn.button_pressed = (name == editor.tool_name)
		
		# Apply icon from the tool instance if available
		var tool := editor.registry.create_tool(name)
		if tool and tool.icon:
			btn.icon = tool.icon
		
		btn.pressed.connect(_on_tool_ui_selected.bind(name, tool_container))
...
func _on_brush_ui_selected(name: String, brush_container) -> void:
	editor.set_active_brush(name)
	for child in brush_container.get_children():
		if child is Button:
			child.button_pressed = (child.text.to_lower() == name)


func _on_tool_ui_selected(name: String, tool_container) -> void:
	editor.set_active_tool(name)
	for child in tool_container.get_children():
		if child is Button:
			child.button_pressed = (child.text.to_lower() == name)


func _on_brush_changed(name: String, brush_container) -> void:
	for child in brush_container.get_children():
		if child is Button:
			child.button_pressed = (child.text.to_lower() == name)


func _on_tool_changed(name: String, tool_container) -> void:
	for child in tool_container.get_children():
		if child is Button:
			child.button_pressed = (child.text.to_lower() == name)
