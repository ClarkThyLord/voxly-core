...
## hovered position on mouse press (no drag, no stroke). This is the
## behavior for brushes with requires_drag = false and continuous = false.
...
func refresh_preview(hit: Dictionary) -> void:
	...
	# the ghost is always visible while hovering. However, tools that only act on
	# existing voxels (like select) filter positions at commit time in work().
	# Positions outside the model shape are clipped so no ghost shows outside.
